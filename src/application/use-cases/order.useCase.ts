import {
  ...
  IOrderStatusEnum,
  IPaymentType,
} from "../../domain/interfaces/order.interface.js";
...
import {
  ...
  getPidgePayload,
  cancelPidgeOrder,
} from "../services/pidge.service.js";
...
import { extractQueryOptions } from "../../infrastructure/db/helper/utils.helper.js";
...
const calculateMartItemsTotal = async (
  ...
  const baseItems = await martProductRepo.getMartProductsForOrder(
    martProductIds
  );
  ...
  const enrichedItems: IOrderFoodItem[] = items.map((cartItem) => {
    ...
    if (!match) {
      console.error(
        `Mart Product ${cartItem.foodItemId} not found in baseItems:`,
        baseItems.map((item) => ({
    ...
    const enrichedItem = {
      foodItemId:
        match.martProductId || match._id?.toString() || cartItem.foodItemId,
...
const getPickupLocation = async (restaurantId: string) => {
  const restaurantAddress = await restaurantRepo.getRestaurantLocationById(
    restaurantId
  );
...
const getMartPickupLocation = async (martStoreId: string) => {
...
const getMartStorePidgePayload = (
  ...
  const totalBillAmount = foodItems.reduce(
  ...
  // Handle both lat/lng and latitude/longitude formats
  const dropoffLat =
    (location.dropoff as any).lat || (location.dropoff as any).latitude;
  const dropoffLng =
    (location.dropoff as any).lng || (location.dropoff as any).longitude;
  ...
  const addressLine1 =
    pickup.martStoreAddress.address?.no ||
    pickup.martStoreAddress.no ||
    pickup.martStoreAddress.street?.split(",")[0] ||
    "N/A";
  const addressLine2 =
    pickup.martStoreAddress.address?.street ||
    pickup.martStoreAddress.street ||
    pickup.martStoreAddress.area ||
    "N/A";
  const city =
    pickup.martStoreAddress.address?.city ||
    pickup.martStoreAddress.city ||
    "N/A";
  const state =
    pickup.martStoreAddress.address?.state ||
    pickup.martStoreAddress.state ||
    "N/A";
  const country =
    pickup.martStoreAddress.address?.country ||
    pickup.martStoreAddress.country ||
    "India";
  const pincode =
    pickup.martStoreAddress.address?.pincode ||
    pickup.martStoreAddress.pincode ||
    "600001";
  ...
  console.log("🔍 [MART STORE DEBUG] Mart store address data:", {
    ...
    street:
      pickup.martStoreAddress.address?.street || pickup.martStoreAddress.street,
    ...
    state:
      pickup.martStoreAddress.address?.state || pickup.martStoreAddress.state,
    country:
      pickup.martStoreAddress.address?.country ||
      pickup.martStoreAddress.country,
    pincode:
...
export const OrderUseCase = {
  ...
  createOnlineOrder: async (data: CreateOrderParams) => {
    const { userId, restaurantId, orderNotes, items, location, deliveryFee } =
      data;
    ...
    const pricing = isMartStoreOrder
      ? await calculateMartTotalPricing(
          itemsTotal,
          data.deliveryFee,
          data.discount || null,
          restaurantId
  ...
  verifyPaymentAndCreateOrder: async ({
    ...
    const isPaymentValid = verifyOrderService({
      orderId,
      paymentId,
      signature,
    });
    ...
    const orderNotes =
      typeof notes.orderNotes === "string" ? notes.orderNotes : "";
    ...
    const discount = notes.discount
      ? typeof notes.discount === "string"
        ? JSON.parse(notes.discount)
        : notes.discount
      : null;
    ...
    let pidgeStatus: "cancelled" | "pending" | "fulfilled" | "completed" =
      "pending";

    try {
      console.log("🔍 [ORDER USE CASE] Starting Pidge order creation...");
      console.log(
        "🔍 [ORDER USE CASE] Order type:",
        isMartStoreOrder ? "Mart Store" : "Restaurant"
      );
      ...
      console.log(
        "🔍 [ORDER USE CASE] Food items count:",
        enrichedItems.length
      );

      const pidgePayload = isMartStoreOrder
        ? getMartStorePidgePayload(
            pickup,
            admin,
            location,
            user as IUser,
      ...
      console.log(
        "🔍 [ORDER USE CASE] Pidge payload created:",
        JSON.stringify(pidgePayload, null, 2)
      );
      ...
      console.log("✅ [ORDER USE CASE] Pidge order created successfully:", {
        pidgeOrderId,
        sourceOrderId,
      });
      ...
      try {
        ...
        if (
          statusFromPidge &&
          ["cancelled", "pending", "fulfilled", "completed"].includes(
            statusFromPidge
          )
        ) {
          pidgeStatus = statusFromPidge as
            | "cancelled"
            | "pending"
            | "fulfilled"
            | "completed";
        ...
        console.log(
          "✅ [ORDER USE CASE] Pidge order status retrieved:",
          pidgeStatus
        );
      } catch (statusError) {
        console.error(
          "❌ [ORDER USE CASE] Failed to get Pidge order status:",
          statusError
        );
        pidgeStatus = "pending";
      }
      ...
      console.log(
        "✅ [ORDER USE CASE] Pidge integration completed successfully:",
        { pidgeOrderId, sourceOrderId, pidgeStatus }
      );
    } catch (pidgeError: any) {
      console.error(
        "❌ [ORDER USE CASE] Failed to create Pidge order:",
        pidgeError
      );
      ...
      console.log(
        "⚠️ [ORDER USE CASE] Continuing with order creation without Pidge integration"
      );
    }
    ...
    if (!recentOrderNotifications.has(userNotificationKey)) {
      ...
      sendUserNotification(userId, {
        ...
        subMessage: `Order #${order.orderId} from ${
          restaurantId.startsWith("mart_") ? "Instamart" : "Restaurant"
        ...
        metadata: {
          ...
          restaurantName: restaurantId.startsWith("mart_")
            ? "Instamart"
            : "Restaurant",
      ...
      try {
        ...
        console.error(
          `❌ Failed to send FCM notification after order placement:`,
          fcmError
        );
    ...
    if (isMartStore) {
      ...
      if (!recentOrderNotifications.has(martStoreNotificationKey)) {
        ...
        try {
          ...
          console.error(
            `❌ Failed to send FCM notification to mart store:`,
            fcmError
          );
          // Don't throw - order was created successfully
        }
      ...
      if (!recentOrderNotifications.has(restaurantNotificationKey)) {
        ...
        try {
          ...
          console.error(
            `❌ Failed to send FCM notification to restaurant:`,
            fcmError
          );
          // Don't throw - order was created successfully
        }
  ...
  getAllOrders: async (filter: Record<string, any> = {}) => {
    // const filteredQuery = extractQueryOptions(filter)
  ...
  createCODOrder: async (data: CreateOrderParams) => {
    const { userId, restaurantId, items, location, orderNotes, discount } =
      data;
    ...
    const pricing = await calculateTotalPricing(
      itemsTotal,
      deliveryFee,
      discount || null,
      restaurantId
    ...
    const order = await orderRepo.createOrder({
      ...
      pricing,
      delivery: {
        location: { ...location, pickup: pickupLocation, distance: distance },
      },
  ...
  updateOrderStatus: async (orderId: string, status: IOrderStatusEnum) => {
    ...
    if (oldStatus === status) {
      console.log(
        `✅ Order status is already ${status} for order ${orderId}. Skipping update and notifications.`
      );
      return oldOrder;
    }
    ...
    if (!userId) {
      console.warn(
        `⚠️ [UPDATE ORDER STATUS] No userId found for order ${orderId}, skipping user notifications`
      );
    }

    if (!restaurantId) {
      console.warn(
        `⚠️ [UPDATE ORDER STATUS] No restaurantId found for order ${orderId}, skipping restaurant notifications`
      );
    }
    ...
    try {
      ...
      const restaurant = restaurantId
        ? await restaurantRepo.getRestaurantLocationById(restaurantId)
        : null;
      const restaurantName =
        restaurantId && restaurantId.length >= 4
          ? `Restaurant ${restaurantId.slice(-4)}`
          : "Restaurant";
      const foodItems = updatedOrder.foodItems || [];
      const foodNames = foodItems
        .map((item) => item.name || item.foodItemId)
        .slice(0, 3);
      const foodSummary =
        foodNames.length > 3
          ...
          case IOrderStatusEnum.CONFIRMED:
            return {
              ...
              restaurant: {
                ...
                } confirmed. Customer: ${user?.name || "Customer"}`,
                theme: "success",
                emoji: "✅",
                priority: "normal",
              },
          ...
          case IOrderStatusEnum.PREPARING:
            return {
              ...
              restaurant: {
                ...
                subMessage: `Started preparing order #${
                  updatedOrder.orderId
                } for ${user?.name || "Customer"}`,
                theme: "info",
                emoji: "👨‍🍳",
                priority: "normal",
              },
          ...
          case IOrderStatusEnum.IN_TRANSIT:
            return {
              ...
              restaurant: {
                ...
                subMessage: `Order #${
                  updatedOrder.orderId
                } is being delivered to ${user?.name || "Customer"}.`,
                theme: "info",
                emoji: "🚚",
                priority: "normal",
              },
          ...
          case IOrderStatusEnum.DELIVERED:
            return {
              ...
              restaurant: {
                ...
                subMessage: `Order #${
                  updatedOrder.orderId
                } delivered successfully to ${user?.name || "Customer"}.`,
                theme: "success",
                emoji: "🎊",
                priority: "normal",
              },
      ...
      if (userId) {
        console.log(
          `📧 [NOTIFICATION] Preparing to notify user ${userId} about order ${orderId} status change to ${status}`
        );
        ...
        if (!recentOrderNotifications.has(userStatusNotificationKey)) {
          ...
          try {
            ...
            console.log(
              `✅ [NOTIFICATION] Socket notification sent to user ${userId}`
            );
          } catch (socketError) {
            console.error(
              `❌ [NOTIFICATION] Failed to send socket notification to user ${userId}:`,
              socketError
            );
          ...
          if (user?.fcmTokens && user.fcmTokens.length > 0) {
            console.log(
              `📱 [NOTIFICATION] User ${userId} has ${user.fcmTokens.length} FCM token(s), sending notification...`
            );
            try {
              ...
              if (result) {
                console.log(
                  `✅ [NOTIFICATION] FCM notification sent to user ${userId}: ${result.successCount} success, ${result.failureCount} failed`
                );
              } else {
                console.warn(
                  `⚠️ [NOTIFICATION] FCM notification returned null for user ${userId}`
                );
              }
            } catch (fcmError) {
              console.error(
            ...
            console.warn(
              `⚠️ [NOTIFICATION] User ${userId} has no FCM tokens. User object:`,
              user ? "found" : "not found",
              user?.fcmTokens ? `${user.fcmTokens.length} tokens` : "no tokens"
            );
          }
          ...
          try {
            if (userId) {
              ...
              console.log(
                `✅ [NOTIFICATION] Database notification created for user ${userId}`
              );
            }
          } catch (notifError) {
            console.error(
              `❌ [NOTIFICATION] Failed to create database notification for user ${userId}:`,
              notifError
            );
          ...
          console.log(
            `⏭️ [NOTIFICATION] Skipping duplicate notification for user ${userId}, order ${orderId}, status ${status}`
          );
        }
      } else {
        console.warn(
          `⚠️ [NOTIFICATION] No userId found for order ${orderId}, cannot send user notification`
        );
      }
        ...
        if (isMartStore) {
          ...
          if (!recentOrderNotifications.has(martStoreStatusNotificationKey)) {
            ...
            try {
              ...
              console.error(
                `❌ [NOTIFICATION] Failed to send socket notification to mart store ${restaurantId}:`,
                socketError
              );
              // Continue even if socket notification fails
            }
            ...
            const martStoreAdmin =
              await martStoreAdminRepo.findByMartStoreAdminByMartStoreId(
                restaurantId
              );
            if (
              martStoreAdmin?.fcmTokens &&
              martStoreAdmin.fcmTokens.length > 0
            ) {
              try {
                await sendFCMNotification({
                  tokens:
                    martStoreAdmin.fcmTokens?.map(
                      (token: IFCM) => token.token
                    ) || [],
                ...
                console.error(
                  `❌ Failed to send FCM notification to mart store admin:`,
                  fcmError
                );
          ...
          if (!recentOrderNotifications.has(restaurantStatusNotificationKey)) {
            ...
            try {
              ...
              console.error(
                `❌ [NOTIFICATION] Failed to send socket notification to restaurant ${restaurantId}:`,
                socketError
              );
              // Continue even if socket notification fails
            }
            ...
            if (
              restaurantAdmin?.fcmTokens &&
              restaurantAdmin.fcmTokens.length > 0
            ) {
              try {
                await sendFCMNotification({
                  tokens:
                    restaurantAdmin.fcmTokens?.map(
                      (token: IFCM) => token.token
                    ) || [],
                  title: notificationData.restaurant.message,
                  body: notificationData.restaurant.subMessage,
                ...
                console.error(
                  `❌ Failed to send FCM notification to restaurant admin:`,
                  fcmError
                );
                // Don't throw - order status was updated successfully
              }
      ...
      console.log(
        `✅ Order status updated to ${status} for order ${orderId}. Notifications sent to user and ${
          isMartStore ? "mart store" : "restaurant"
        }.`
      );
    } catch (error) {
      console.error(
  ...
  customerFeedback: async (orderId: string, feedback: IOrderFeedback) => {
    const customer_feedback = await orderRepo.setOrderFeedback(
      orderId,
      feedback
    );
  ...
  cancelOrder: async (orderId: string) => {
    ...
    const cancelledOrder = await orderRepo.updateOrderStatus(
      orderId,
    ...
    try {
      ...
      if (cancelledOrder.delivery?.pidge?.pidgeId) {
        ...
        console.log(
          `✅ [ORDER CANCELLATION] Successfully cancelled Pidge order for pidgeId: ${cancelledOrder.delivery.pidge.pidgeId}`
        );
      } else {
        console.log(
          `⚠️ [ORDER CANCELLATION] No Pidge ID found for orderId: ${orderId}, skipping Pidge cancellation`
        );
      }
    } catch (pidgeError: any) {
      console.warn(
        `⚠️ [ORDER CANCELLATION] Pidge cancellation failed for orderId: ${orderId}, but order was cancelled in system:`,
        pidgeError?.response?.data || pidgeError.message
      );
    ...
    try {
      ...
      if (
        order.refIds?.restaurantId &&
        !order.refIds.restaurantId.startsWith("mart_")
      ) {
        const restaurantAdmin =
          await restaurantAdminRepo.findByRestaurantAdminByRestaurantId(
            order.refIds.restaurantId
        ...
        if (restaurantAdmin) {
          ...
          if (
            ...
            await sendFCMNotification({
              tokens: restaurantAdmin.fcmTokens.map(
                (token: IFCM) => token.token
              ),
              title: "Order Cancelled ❌",
  ...
  getOrdersWithUserAndRestaurantDetails: async (
    ...
    const enrichedOrders = await Promise.all(
      orders.map(async (order) => {
        const [user, restaurant] = await Promise.all([
          userRepo.findByUserId(order.refIds?.userId as string),
          restaurantRepo.findByRestaurantId(order.refIds?.restaurantId as string),
        ]);

        return {
          ...(order.toObject?.() ?? order), // safely handle both mongoose doc or plain obj
          user: user
            ? {
                userId: user.userId,
                name: user.name,
  ...
  getOrdersWithRestaurantDetails: async (filter: FilterQuery<IOrder> = {}) => {
    ...
    console.log(
      `Found ${orders.length} orders, fetching restaurant/mart store details...`
    );
      ...
      orders.map(async (order) => {
        try {
          ...
          if (isMartStore) {
            ...
            const martStore = await martStoreRepo.findByMartStoreId(
              restaurantId
            );
          ...
          console.error(
            `❌ Error fetching restaurant details for order ${order.orderId}:`,
            error
          );
          return {
            ...order.toObject(),
    ...
    console.log(
      `✅ Successfully enriched ${enrichedOrders.length} orders with restaurant details`
    );
    return enrichedOrders;
  },
  ...
  getUserOrdersWithRestaurantDetails: async (userId: string) => {
    ...
    return await OrderUseCase.getOrdersWithRestaurantDetails({
      "refIds.userId": userId,
    });
  },
  ...
  getRestaurantOrdersWithDetails: async (restaurantId: string) => {
    console.log(
      "🔍 Fetching orders for restaurant with details:",
      restaurantId
    );
    return await OrderUseCase.getOrdersWithRestaurantDetails({
